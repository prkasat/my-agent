...
describe("SessionManager", () => {
	...
	describe("branching", () => {
		...
		it("Tier-1: navigateBranch records summary on the new branch and returns the abandoned tail", () => {
			...
			const summary = manager.getEntry(defined(result.summaryEntryId));
		...
		it("Tier-1: navigateBranch rolls leaf back when summary persistence throws", () => {
			...
			expect(() => manager.navigateBranch(d, "Should fail")).toThrow(/simulated fs failure/);
		...
		it("Codex-pass4-fix: navigateBranch under withLock truncates disk so reopen sees the original leaf", async () => {
			...
			const d = manager.appendMessage({ role: "user", content: "D", timestamp: Date.now() });
			const sessionFile = defined(manager.getSessionFile());
	...
	describe("persistence", () => {
		...
		it("regression (pass-12): flush() forces deferred entries to disk for stub-only sessions", () => {
			...
			const user = entries.find((e): e is MessageEntry => e.type === "message" && e.message.role === "user");
			expect(user).toBeDefined();
			expect(typeof user?.message.content === "string" ? user?.message.content : "").toBe("lone prompt");
		});
		...
		it("should load existing session", () => {
			...
			const sessionFile = defined(manager1.getSessionFile());
	...
	describe("Tier-2: extension entries", () => {
		...
		const itWithPosixChmod = process.platform === "win32" ? it.skip : it;

		itWithPosixChmod(
			"Tier-2 pass-15 regression: SessionManager.flush() first-flush retry does not duplicate",
			async () => {
		...
		itWithPosixChmod(
			"Tier-2 pass-14 regression: forced first-flush is atomic — failed write + retry doesn't duplicate",
			async () => {
...
describe("SessionManager.withLock cross-process behavior", () => {
	...
	it("regression: reloads disk state at lock acquisition so concurrent writers see each other", async () => {
		...
		const sessionFile = defined(a.getSessionFile());
	...
	it("regression (pass-11): branch() before withLock survives the locked reload", async () => {
		...
		const sessionFile = defined(session.getSessionFile());
		...
		const branched = defined(onDisk.find((e) => e.id === appendedId));
	...
	describe("Tier-4: labels", () => {
		...
		it("Codex-pass12-fix: newSession() resets label state and freshness baseline", () => {
			...
			const labelEntry = manager
				.getEntries()
				.find((e): e is Extract<SessionEntry, { type: "label" }> => e.type === "label" && e.targetId === u2);
		...
		it("Codex-pass10-fix: forking an unlabeled v1 session keeps the fork at v1", () => {
			...
			const forkHeader = JSON.parse(fs.readFileSync(defined(forkPath), "utf-8").split("\n")[0]);
		...
		it("Codex-pass13-fix: v1 promotion does not clobber peer writes", () => {
			...
			expect(() => manager.appendLabelChange("u1", "marker")).toThrow(/changed externally/);
		...
		it("Codex-pass3-fix: label move is one atomic LabelEntry with displaces", () => {
			...
			const moveEntry = defined(manager.getEntry(moveId));
		...
		it("Codex-pass3-fix: forking the actual leaf of the old branch does not resurrect", () => {
			...
			expect(forkPath).toBeTruthy();
			const reopened = SessionManager.open(defined(forkPath));
		...
		it("Codex-pass1-fix: forkSession remaps LabelEntry.targetId", () => {
			...
			const forkPath = manager.forkSession();
			expect(forkPath).toBeTruthy();
			expect(existsSync(defined(forkPath))).toBe(true);
			...
			expect(reopened.getEntry(defined(labelTarget))).toBeTruthy();
