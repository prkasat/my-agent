...
async function buildSessionInfo(filePath: string): Promise<SessionInfo | null> {
	try {
		...
		for (const entry of entries) {
			...
			if (!Number.isNaN(entryTime) && entryTime > lastTimestamp) {
...
export class SessionManager {
	...
	private leafSelectedByUser = false;
		...
		if (!this.flushed) {
			...
			if (this.lastLoadedFileSig !== null && existsSync(this.sessionFile)) {
				if (this.fileChangedSinceLastLoad()) {
					throw new Error("Session file changed externally during write; reload or wrap in withLock before retrying");
				}
			}
			this.rewriteFile();
			this.flushed = true;
	...
	private fileChangedSinceLastLoad(): boolean {
		...
		const st = statSync(this.sessionFile);
		return st.mtimeMs !== this.lastLoadedFileSig.mtimeMs || st.size !== this.lastLoadedFileSig.size;
	}
	 ...
	 */
	appendBranchSummary(fromId: string, summary: string, details?: BranchSummaryDetails): string {
	 ...
	 */
	appendExtension(namespace: string, payload: unknown, subtype?: string): string {
	...
	navigateBranch(
		...
		const { entries: abandonedEntries, commonAncestorId } = collectEntriesForBranchSummary(this, oldLeafId, targetId);
		...
		const shouldWriteSummary = summary && !!oldLeafId && commonAncestorId !== null && abandonedEntries.length > 0;
	...
	async withLock<T>(fn: () => Promise<T>): Promise<T> {
		...
		return withCrossProcessLock(sessionFile, async () => {
			...
			if (result.status === "ok") {
				...
				if (intendedLeafSelected && intendedLeafId !== null && this.byId.has(intendedLeafId)) {
	 ...
	 */
	static async withSessionLock<T>(sessionFile: string, fn: () => Promise<T>): Promise<T> {
