 ...
 */
export type CompactionCallback = (result: CompactionCallbackResult) => void;
 ...
 * ```
 */
export function createAutoCompactor(options: AutoCompactorOptions): AutoCompactorTransform {
	...
	const transformContext = (async (context: AgentContext, signal?: AbortSignal): Promise<AgentContext> => {
		...
		try {
			const apiKey = options.getApiKey ? await options.getApiKey(context.model.provider) : undefined;
			...
			const availableForKept = Math.max(0, Math.floor((contextWindow - effectiveReserve) * 0.75));
			...
			const forceProgress = measurement.lastUsageIndex !== null && measurement.usageTokens > limit;
...
export function createAutoCompactorWithPersistence(
	...
	const wrapped = async function transformContext(context: AgentContext, signal?: AbortSignal): Promise<AgentContext> {
		...
		if (compactionResult !== undefined && compactionResult.cutIndex > 0) {
			...
			if (firstKeptEntryId) {
				try {
					...
					const persist = () =>
						options.sessionManager.appendCompaction(summary, firstKeptEntryId, tokensBefore, details);
