...
export async function loadSettings(cwd: string): Promise<Settings> {
	...
	return normalizeSettings(
		deepMerge(DEFAULTS as unknown as Record<string, unknown>, userSettings, projectSettings) as unknown as Settings,
	);
}
...
async function loadJsonFile(filePath: string): Promise<Record<string, unknown>> {
	try {
		...
		if (await backupCorruptedJsonFile(filePath)) {
			trace("resources", "settings.recovered_corrupt_file", { filePath });
		} else {
			trace("resources", "settings.load_empty", {
				filePath,
...
async function backupCorruptedJsonFile(filePath: string): Promise<boolean> {
	try {
		const content = await fs.readFile(filePath, "utf-8");
		try {
			JSON.parse(content);
...
export async function saveSettings(
	...
	const dir =
		scope === "user" ? path.join(process.env.HOME || ".", ".my-agent") : path.join(cwd || process.cwd(), ".my-agent");
	...
	const merged = normalizeSettings(deepMerge(existing, settings as Record<string, unknown>) as unknown as Settings);
