...
import { resolveConfiguredModel, listModelAvailability } from "../../src/runtime/model-registry.js";
...
describe("model registry", () => {
  ...
  it("throws a helpful error when no authenticated models are available", async () => {
    ...
    await expect(resolveConfiguredModel(settings, authStorage)).rejects.toThrow(/No authenticated models are available/);
  });
});
