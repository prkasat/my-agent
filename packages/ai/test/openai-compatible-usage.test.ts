...
describe("openai-compatible usage accounting", () => {
	...
	it("Codex-pass6-fix: outbound request includes stream_options.include_usage", async () => {
		...
		globalThis.fetch = (async (_url: any, init: any) => {
			...
			const data = ssePayload([{ choices: [], usage: { prompt_tokens: 1, completion_tokens: 1 } }]);
			return new Response(
	...
	it("includeRealCost adds usage:{include:true} to outbound request", async () => {
		...
		globalThis.fetch = (async (_url: any, init: any) => {
			...
			return new Response(
				new ReadableStream<Uint8Array>({
					start(controller) {
						controller.enqueue(ssePayload([{ choices: [], usage: { prompt_tokens: 1, completion_tokens: 1 } }]));
						controller.close();
					},
				}),
				{ status: 200 },
