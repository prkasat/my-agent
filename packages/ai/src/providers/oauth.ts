...
async function startLocalCallbackServer(options: {
  ...
  const successHtml =
    options.successHtml ?? "<h1>Login successful!</h1><p>You can close this window.</p>";
  const errorHtml =
    options.errorHtml ?? ((message: string) => `<h1>Login failed</h1><p>${escapeHtml(message)}</p>`);
  ...
  return {
    ...
    async login(callbacks: OAuthLoginCallbacks): Promise<OAuthCredentials> {
      ...
      try {
        callbacks.onAuth({
          url: url.toString(),
          instructions: "Complete the OpenAI login in your browser. If the callback fails, paste the full redirect URL.",
        });
